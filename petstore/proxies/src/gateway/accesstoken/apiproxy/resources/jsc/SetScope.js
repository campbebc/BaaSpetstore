 ...
 * belongs to.
 */
var response = JSON.parse(context.getVariable("petstore.response"));
...
if (response.statusCode == 400) {
    ...
    context.setVariable("petstore.auth.status.code", 400);
} else {
    try {
        /**
        ...
        if (response.user.user_groups && response.user.user_groups.length > 0) {
            ...
            context.setVariable("petstore.user.scope", userScope);
    	    context.setVariable("petstore.auth.status.code", 200);
        }
    } catch (error) {
...
function setResponse(status, message, detail) {
	...
	context.setVariable("petstore.auth.status.code", status);
	context.setVariable("petstore.error.message", message);
	context.setVariable("petstore.error.header.Content-Type", "application/json");
	context.setVariable("petstore.error.content", errorContent);
}